...
| Consensus | N/A（非计算域） | — | — | — | — |
| Knowledge Layer | 引用层（不产生计算输出） | Architecture **FROZEN**（KB-001~020）; Pipeline **VALIDATED**（Phase 7.0）; Corpus **PARTIAL**（Ziwei, 41 节点 + 18 关系 + 7 引用, Phase 7.1.3）; Provenance **AUDITED** | — | — | 10（pipeline 回归） |
