...
| Consensus | N/A（非计算域） | — | — | — | — |
| Knowledge Layer | 引用层（不产生计算输出） | Architecture **FROZEN**（KB-001~020）; Pipeline **VALIDATED**（Phase 7.0）; Corpus **PARTIAL**（Ziwei, 53 节点 + 23 关系 + 8 引用, Phase 7.1.4B）; Provenance **AUDITED** | — | — | 10（pipeline 回归） |
