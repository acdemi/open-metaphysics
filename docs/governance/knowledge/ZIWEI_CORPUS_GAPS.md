...
| GAP-12 | auxiliary_star（紫微辅星）Tier 1 主源**原文未入库** —— `source_ziwei_01`《紫微斗数全书》source_01.yaml 明记「原文不在仓库内（版权/体积考虑）; url=null; 引用以章节/页码为据」; `source_ziwei_02`《紫微斗数全集》GAP-01 待获取 | auxiliary_star 首批节点**0 生产**（7.2B）—— 辅星语义（文昌/文曲/左辅/右弼/天魁/天钺/禄存/擎羊/陀罗/火星/铃星）的权威定义在紫微全书, 无原文则无法逐字核验; 按"不可核实→不生产"原则拒绝 | Phase 7.2B | **待源文本可达**（需: 紫微全书原文入库, 或 紫微全集获取后多源交叉, 或 用户授权降级策略） |
| GAP-13 | shen_sha（神煞）Tier 1 原文**逐字核验未达成** —— `source_sanming_01`《三命通会》/`source_bazi_01`《渊海子平》均已登记（wikisource 通行本, 7.1.5 曾成功核验卷二·三刑/六害）; 但 7.2B 会话内对 wikisource 的 webfetch 2 次失败（timeout / transport error）, 神煞章节（论羊刃/论驿马/论亡神劫煞/论天乙贵人/论禄 等）verbatim 原文未能获取 | shen_sha 首批节点**0 生产**（7.2B）—— 来源已登记且理论上可达, 但本会话无法逐字核验; 按"不虚构"原则拒绝以训练知识/网络常见度替代原文 | Phase 7.2B | **待原文核验**（需: webfetch 可用后重试, 或 紫微全书/三命通会原文入库, 或 用户提供可核验文本） |
