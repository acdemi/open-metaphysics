# Ziwei Behavior Contract (Draft)

> **状态**: **DRAFT** —— Freeze Review 已 PASS（`ZIWEI_FREEZE_REVIEW.md`,
> 2026-08-13），本文档为正式冻结前置产物
...
> **Golden Vectors**: `docs/ziwei/golden_vectors.json`（24, candidate → 冻结后为
> normative fixtures）
...
> `ZIWEI_CROSS_DOMAIN_BOUNDARIES.md`
...
| vector_store | docs/ziwei/golden_vectors.json（24, candidate） |
| schema_ref | ZiweiInput / ZiweiChart / Palace（agents/ziwei.py）+ SCHEMAS.md §3.2 |
| test_refs | tests/test_ziwei.py（33）+ test_ziwei_golden_vectors.py（7） |
...
- **Definition**: `docs/ziwei/golden_vectors.json` 24 向量为规范回归装置
...
## 4. 跨域边界声明（引用 ZIWEI_CROSS_DOMAIN_BOUNDARIES.md）
...
## 5. 冻结前置条件（Review Board 确认清单）

1. Freeze Review PASS（✅ 已完成, `ZIWEI_FREEZE_REVIEW.md`, 2026-08-13）
2. 边界裁定完成（✅ A-3/A-4/A-6/A-7 全部 FROZEN, 显式声明）
3. 24 向量充分性（✅ 已确认, 覆盖 17/17 规则 + A-1/A-2 专项）
4. 单元测试 33 例 + 黄金向量测试 7 例（✅ 全部通过, 全量 585/585）
5. 文档勘误 E-1 已记录（✅ readiness 标注"木三局" → 实际"土5局",
   见 FREEZE_REVIEW §3; 正式冻结以本契约 + golden_vectors.json 为准）
6. 本草案评审通过 → 版本 0.1.0-draft → **1.0.0 Frozen**（须人工 Gate 授权）
