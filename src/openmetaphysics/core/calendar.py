"""Deterministic luni-solar calendar primitives.

Solar-term times are computed from the Sun's apparent geocentric ecliptic
longitude (Meeus, truncated). No ephemeris files, no network, no LLM. All
results are reproducible from input alone.

Accuracy of the solar longitude is ~0.01 deg (well under a minute of time),
which is ample for day-level Bazi boundaries.
"""
...
from .models import BAZI_MONTH_BOUNDARIES, SOLAR_TERMS_24
...
# ---------------------------------------------------------------------------
# Solar longitude (Meeus, truncated)
# ---------------------------------------------------------------------------
...
def solar_term_time(year: int, target_lon: int) -> datetime:
    ...
    for day in range(0, 367):
        ...
        # crossing: w wraps from ~360 down to ~0
...
def solar_terms_for_year(year: int) -> dict[str, datetime]:
    return {name: solar_term_time(year, lon) for name, _is_jie, lon in SOLAR_TERMS_24}
...
def sexagenary_day_index(year: int, month: int, day: int) -> int:
    """Sexagenary index 0..59 (甲子=0) for a Gregorian date."""
    return (julian_day_number(year, month, day) + 49) % 60


# ---------------------------------------------------------------------------
# Bazi month boundaries (the 12 节)
# ---------------------------------------------------------------------------
@lru_cache(maxsize=256)
def _month_boundaries(year: int) -> list[tuple[datetime, int, str]]:
    ...
    for term_name, lon, branch_idx in BAZI_MONTH_BOUNDARIES:
        # a 节 in early Jan belongs to the year that started the previous Lichun;
        # scanning the Gregorian year catches them via solar_term_time(year, lon).
        t = solar_term_time(year, lon)
        out.append((t, branch_idx, term_name))
...
def month_boundary_before(born_at: datetime) -> tuple[int, str, datetime]:
    ...
    candidates = _month_boundaries(y - 1) + _month_boundaries(y) + _month_boundaries(y + 1)
...
# ---------------------------------------------------------------------------
# Solar → Lunar conversion via sxtwl
# ---------------------------------------------------------------------------
def solar_to_lunar(year: int, month: int, day: int) -> tuple[int, int, int, bool]:
    ...
    - lunar_month is 1..12 (leap month keeps same number as non-leap)
    - is_leap_month is True if this is the leap (闰) month
    """
    import sxtwl
